...
<button v-on:click="showForm(comp)">Edit</button>
    ...
    <dialog id="form-edit">
      <form method="dialog">
        ...
        <p>competency name: <input type="text" v-model="currentComp.name"></p>
         <p>competency description: <input type="text" v-model="currentComp.descr"></p>
       
        
       <button v-on:click="updateRecipe(currentComp)">Update</button>
        <button v-on:click="destroyRecipe(currentComp)">Destroy</button>
        <button>Close</button>
      </form>
    </dialog>
    </tr>
    ...
    <form action = "#">
        ...
        <p>competency name: <input type="text" v-model="newName"></p>
         <p>competency description: <input type="text" v-model="newDescr"></p>
      ...
      axios.post("http://localhost/project2/data_p2.php", params).then(response => {
        console.log("Success updating recipe", response.data);
         for (let i = 0; i < this.comps.length; i++){
          if (response.data[0].competency_id === this.comps[i].competency_id){
